...
| `pre-commit-advisory.yml` | Runs the caller's own `.pre-commit-config.yaml` hooks against the **PR diff** (`--from-ref`/`--to-ref`), posts a sticky PR comment with the output, and fails the job so the ❌ is visible. **Non-blocking** — deliberately not in any repo's required checks; harden by adding the job to branch protection in the GitHub UI, no YAML change needed. Push-to-default-branch runs only seed the hook-env cache. Toolchains the hooks shell out to are opt-in, each defaulting to false: `setup-dotnet` (+ `dotnet-version`), `setup-node-pnpm` (+ `node-version`), `setup-terraform` (+ `terraform-version`, which defaults to the repo's `.tool-versions` pin), and `private-modules` for hooks resolving private Go modules. |
